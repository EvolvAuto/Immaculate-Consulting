...
//
// AiDraftPriorityPopulations renders the priority_populations_addressed
// section from cmp-draft-care-plan v4 - shows which NC DHHS priority
// populations the AI addressed and where, with explicit gap warnings.
// ===============================================================================
...
export default function NewPlanModal({ practiceId, userId, onClose, onCreated }) {
  ...
  const save = async () => {
    ...
    if (aiDraft) {
      ...
      payload.priority_populations_addressed = Array.isArray(aiDraft.priority_populations_addressed)
        ? aiDraft.priority_populations_addressed
        : [];
...
function AiDraftPreview({ draft }) {
  ...
  const populations   = Array.isArray(draft.priority_populations_addressed) ? draft.priority_populations_addressed : [];
  ...
  return (
    <div style={{ padding: 12, background: "#fafafa", border: "0.5px solid " + C.borderLight, borderRadius: 8 }}>
      ...
      <AiDraftPriorityPopulations items={populations} />
...
// ---------------------------------------------------------------------------
// AiDraftPriorityPopulations - shows which NC DHHS priority populations the
// AI addressed in this plan, and where (which goal/intervention index covers
// each). Surfaces gaps prominently: if the AI couldn't find a way to address
// a flagged population given available data, that row gets an amber warning
// so the CM knows to add coverage manually before activating.
//
// Renders nothing if the member has no flagged populations (empty array from
// edge function).
// ---------------------------------------------------------------------------
function AiDraftPriorityPopulations({ items }) {
  if (!items || items.length === 0) return null;

  const gaps = items.filter(p =>
    (p.addressed_in_goal_index === null || p.addressed_in_goal_index === undefined) &&
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 8 }}>
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "#166534" }}>
          Priority populations addressed ({items.length})
        </div>
        {gaps.length > 0 && (
          <Badge label={gaps.length + " gap" + (gaps.length === 1 ? "" : "s")} variant="amber" size="xs" />
        )}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {items.map((p, i) => {
          const isGap = (p.addressed_in_goal_index === null || p.addressed_in_goal_index === undefined) &&
                        (p.addressed_in_intervention_index === null || p.addressed_in_intervention_index === undefined);
          const coverage = [];
          if (typeof p.addressed_in_goal_index === "number") {
            coverage.push("Goal " + (p.addressed_in_goal_index + 1));
          }
          if (typeof p.addressed_in_intervention_index === "number") {
            coverage.push("Intervention " + (p.addressed_in_intervention_index + 1));
          }
          return (
            <div key={i} style={{
              padding: "8px 10px",
              background: isGap ? C.amberBg : "#fff",
              border: "0.5px solid " + (isGap ? C.amberBorder : "#bbf7d0"),
              borderRadius: 6,
            }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: p.note ? 4 : 0 }}>
                <span style={{
                  fontSize: 11,
                  fontWeight: 700,
                  padding: "2px 8px",
                  borderRadius: 10,
                  background: isGap ? "#fff" : "#dcfce7",
                  color: isGap ? "#854F0B" : "#166534",
                  border: "0.5px solid " + (isGap ? C.amberBorder : "#bbf7d0"),
                }}>
                  {p.label || p.code || "Unknown"}
                </span>
                {coverage.length > 0 ? (
                  <span style={{ fontSize: 11, color: C.textSecondary, fontWeight: 600 }}>
                    Covered by: {coverage.join(" + ")}
                  </span>
                ) : (
                  <span style={{ fontSize: 11, color: "#854F0B", fontWeight: 600 }}>
                    Not addressed - CM should add coverage before activating
                  </span>
                )}
              </div>
              {p.note && (
