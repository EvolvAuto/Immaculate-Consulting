...
export function TrendChart({ data, unit = "", goalLow, goalHigh, thresholdLow, thresholdHigh, refLow, refHigh, height = 220, color = "#378ADD", higherIsBetter = false }) {
  ...
  // ── Hover handling ──────────────────────────────────────────────────────────
  // Translate mouse position into the nearest data point index.
  // The SVG scales to fit its container via viewBox, so we convert the DOM
  // event coordinates to the SVG's internal coordinate space before snapping.
  const handleMove = (e) => {
  ...
  // Status evaluation (replicates the logic from evaluateMetric but scoped to
  // what TrendChart receives - it only has goal/threshold thresholds, not the
  // full metric object). Falls back to neutral when thresholds aren't provided.
  const statusFor = (val) => {
  ...
  const renderTooltip = () => {
    ...
    // Tooltip dimensions (fixed - we know max content width)
    const tipW = 180;
    ...
    return (
      <g style={{ pointerEvents: "none" }}>
        ...
        {/* Status chip (only rendered when a status applies) */}
        {status.label && (
